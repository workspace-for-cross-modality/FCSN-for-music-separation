import torch
import torchvision
from .networks import Resnet18, AudioVisual5layerUNet, AudioVisual7layerUNet, weights_init, DCGAN_D, Normal_UNet7, Down_channels
...
class ModelBuilder():
    ...
    def build_unet(self, unet_num_layers=7, ngf=64, input_nc=1, output_nc=1, weights=''):
        ...
        if len(weights) > 0:
            print('Loading weights for sepration-UNet{}'.format(unet_num_layers))
            net.load_state_dict(torch.load(weights))
        return net

    # builder for audio classifier stream
    def build_classifier(self, pool_type='maxpool', num_of_classes=15, input_channel=1, weights=''):
        pretrained = True
        original_resnet = torchvision.models.resnet18(pretrained)
    ...
    def build_refine(self, opt, ngf=64, input_nc=1, output_nc=1, weights=''):
        net = Normal_UNet7(opt, ngf, input_nc, output_nc)

        net.apply(weights_init)
        if len(weights) > 0:
            print('Loading weights for refine-UNet7')
            net.load_state_dict(torch.load(weights))
        return net

    def build_down_channels(self, weights=''):
        net = Down_channels()
        net.apply(weights_init)
        if len(weights)>0:
            print('Loading weights for down_channels model')
            net.load_state_dict(torch.load(weights))
        return net
