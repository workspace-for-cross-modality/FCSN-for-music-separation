...
def get_frame_root(npy_path):
    ...
    return os.path.join(os.path.dirname(a), 'train_clip_screen_extract')
...
def get_audio_root(npy_path):
    a = os.path.dirname(os.path.dirname(npy_path))
    return os.path.join(os.path.dirname(a), 'train_clip_screen_audio_11025')
...
class AudioVisual_audioset(torchdata.Dataset):
    def __init__(self,mode,opt):
        ...
        h5f_path = os.path.join('/home/mashuo/dataset/audioset_instrument/h5_file', 'audioset_train.h5')
    ...
    def __getitem__(self, index):
        ...
        for n in range(N):
            ...
            detect_bbs = clip_det_bbs[n]
            audios[n] = audio_segment  # make a copy of the audio to mix later

            detect_bbs = detect_bbs[detect_bbs[:,2].argsort()]

            if detect_bbs.shape[0]>2:
                clip_det_bbs[n] = detect_bbs[0:2]
            # 限制只取两类，并且按照概率的高低降序排列，取前两个
