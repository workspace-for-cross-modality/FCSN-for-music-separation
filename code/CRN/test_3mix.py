...
def test_sepration(opt, nets, output_dir, save_files=False):
	...
	audio3_path = os.path.join(opt.data_path, 'solo_audio_resample', opt.video3_ins, opt.video3_name + '.wav')
	...
	for i in range(opt.num_of_object_detections_to_use):
		...
		if clip_det_bbs3.shape[0] == 1:
			frame_path3 = os.path.join(opt.data_path, 'solo_extract', opt.video3_ins, opt.video3_name, "%06d.png" % clip_det_bbs3[0,0])
			detection_bbs_filter_3 = clip_det_bbs3[0]
		elif clip_det_bbs3.shape[0] >= 2:
			hand_npy = os.path.join(opt.data_path,'solo_detect_hand',opt.video3_ins,opt.video3_name + '_hand.npy')
			if os.path.exists(hand_npy):
				hand_bbs = np.load(hand_npy)
			else:
				hand_bbs = np.array([])
			...
			if hand_bbs.shape[0] == 0:
				...
				hand_bb = np.concatenate((hand_bb1, hand_bb2), axis=0)
				sign = True
			detection_bbs_filter_3 = filter_det_bbs(hand_bb, sign, clip_det_bbs3)
			frame_path3 = os.path.join(opt.data_path, 'solo_extract', opt.video3_ins, opt.video3_name,
									   "%06d.png" % clip_det_bbs3[0, 0])

		detection3 = Image.open(frame_path3).convert('RGB').crop((detection_bbs_filter_3[-4],
																  detection_bbs_filter_3[-3],
																  detection_bbs_filter_3[-2],
																  detection_bbs_filter_3[-1]))
		...
		while sliding_window_start + samples_per_window < audio_length:
			...
			objects_visuals_256.append(vision_transform_for_unet(detection3).unsqueeze(0))
			objects_labels.append(torch.FloatTensor(np.ones((1, 1))))
			objects_vids.append(torch.FloatTensor(np.ones((1, 1))))
			...
			sep_audio3[sliding_window_start:sliding_window_end] = sep_audio3[sliding_window_start:sliding_window_end] + reconstructed_signal[2]
			refine_sep3[sliding_window_start:sliding_window_end] = refine_sep3[sliding_window_start:sliding_window_end] + refine_signal[2]
		...
		sep_audio3[-samples_per_window:] = sep_audio3[-samples_per_window:] + reconstructed_signal[2]
		refine_sep3[-samples_per_window:] = refine_sep3[-samples_per_window:] + refine_signal[2]
	...
	output_dir = os.path.join(output_dir, opt.video1_name +'$_VS_$' + opt.video2_name + '$_VS_$' + opt.video3_name)
	...
	if save_files:
		...
		librosa.output.write_wav(os.path.join(output_dir, 'audio3.wav'), audio3, opt.audio_sampling_rate)
	...
	c_reference_sources = np.concatenate((np.expand_dims(audio1, axis=0), np.expand_dims(audio2, axis=0), np.expand_dims(audio3, axis=0)), axis=0)
	c_estimated_sources = np.concatenate((np.expand_dims(separation1, axis=0),
										  np.expand_dims(separation2, axis=0),
										  np.expand_dims(separation3, axis=0)), axis=0)

	c_sdr, c_sir, c_sar = getSeparationMetrics(c_reference_sources, c_estimated_sources)

	r_reference_sources = np.concatenate((np.expand_dims(audio1, axis=0), np.expand_dims(audio2, axis=0), np.expand_dims(audio3, axis=0)), axis=0)
	r_estimated_sources = np.concatenate((np.expand_dims(refine_spearation1, axis=0),
										  np.expand_dims(refine_spearation2, axis=0),
										  np.expand_dims(refine_spearation3, axis=0)), axis=0)
	...
	if opt.visualize_spectrogram:
		...
		audio3_mag = generate_spectrogram_magphase(audio3, opt.stft_frame, opt.stft_hop, with_phase=False)
		...
		utils.visualizeSpectrogram(audio3_mag[0,:,:], os.path.join(output_dir, 'audio3_spec.png'))
...
if __name__ == '__main__':
	...
	checkpoint_weights = './checkpoints/sup11_music21_train_3mix/checkpoint_latest.pth'
	...
	test_name = 'test_sup11_music21_train_3mix_2nd' #opt.test_name
	# output dir
	...
	writer.writerow(['No.','first', 'another','third', 'c_sdr', 'c_sar', 'c_sir','r_sdr', 'r_sar', 'r_sir'])
	...
	for iters in range(10):
		for pair in test_pairs:
			...
			print("|{}|:sep {} with {} with {}, c_sdr:{}, c_sir:{}, c_sar:{}, r_sdr:{}, r_sir:{}, r_sar:{}\n".format(total_tests,
																										  opt.video1_name,
																										  opt.video2_name,
																										  opt.video3_name,
																										  c_sdr,c_sir, c_sar,
																										  r_sdr, r_sir,r_sar))
			writer.writerow([total_tests, opt.video1_ins,opt.video2_ins,opt.video3_ins,opt.video1_name,opt.video2_name,opt.video3_name, c_sdr, c_sir, c_sar, r_sdr, r_sir, r_sar])
