...
def get_frame_root(npy_path):
    ...
    return os.path.join(os.path.dirname(a), 'train_extract')
...
def get_audio_root(npy_path):
    a = os.path.dirname(os.path.dirname(npy_path))
    return os.path.join(os.path.dirname(a), 'train_audio_11025')
...
class AudioVisual_vggsound(torchdata.Dataset):
    def __init__(self,mode,opt):
        super(AudioVisual_vggsound, self).__init__()
        self.mode = mode
        self.opt = opt
        ...
        h5f_path = os.path.join('/home/mashuo/work/study/refine-separation/dataset/vggsound/h5_file', 'vgg_screen_train.h5')
        ...
        for detection in detections:
            ...
            # detection = detection.replace('/train_detect/', '/train_detect_all/')
            self.list_sample.append(detection)
            insname = get_ins_name(detection) #get video id

            if self.detection_dic.__contains__(insname):
                self.detection_dic[insname].append(detection)
            else:
                self.detection_dic[insname] = [detection]
    ...
    def __getitem__(self, index):
        ...
        for n in range(N):
            ...
            det_box = detection_bbs[np.argmax(detection_bbs[:, 2]), :]
            ...
            frame_path = os.path.join(get_frame_root(clip_paths[n]), get_ins_name(clip_paths[n]),
                                      ...
                                      str(int(det_box[0])).zfill(6) + '.png')

            label = det_box[1] - 1  # make the label start from 0
            object_image = Image.open(frame_path).convert('RGB').crop(
                (det_box[-4], det_box[-3], det_box[-2], det_box[-1]))
            ...
            objects_labels.append(label)  # make a copy of the audio spec for each object
